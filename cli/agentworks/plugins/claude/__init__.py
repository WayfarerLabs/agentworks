"""The ``claude`` system plugin: the Claude Code session harness and its
...
``ClaudeCodeHarness`` into ``HARNESS_REGISTRY`` through the ``harness``
...
- The harness row publishes with a ``system-plugin`` origin; a
  ...
  does not propagate), and ``ensure_harness_enabled`` refuses it at session
