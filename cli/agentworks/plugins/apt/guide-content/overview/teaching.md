Use the System plugins roster to discover the installed `apt` plugin and its configured state. The
resource inventory and resource descriptions show the catalog rows, including disabled rows when you
ask for them. After authorization, enable it exactly by adding `apt` to `[plugins].system` while
preserving existing entries, then use the separate verification action.
