...
the `list` and `show` subcommands. Guide-global `--agent/--human` may precede either, and the same
options are accepted on each subcommand; only the single topic on `show` attaches the dynamic topic
completer. The list service validates the complete shell catalog first, so an unrelated malformed or
duplicate shell suppresses topic completion rather than producing a partial candidate stream.
