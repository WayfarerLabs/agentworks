...
DYNAMIC_SNIPPETS: dict[str, str] = {
    ...
    "catalog_entries": (
        "& { agentworks catalog list 2>$null"
    ...
    "consoles": (
        "(agentworks console list 2>$null | Select-Object -Skip 2 |"
        " ForEach-Object { ($_ -split '\\s+')[0] } |"
        ' Where-Object { $_ -like "$wordToComplete*" })'
    ),
    "session_templates": (
...
def generate_powershell(spec: CommandSpec, version: str) -> str:
    ...
    lines.append("#")
    lines.append("# Recommended install:  agentworks completion install --shell powershell")
