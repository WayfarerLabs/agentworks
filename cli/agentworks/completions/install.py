...
def _install_bash(script: str) -> None:
    ...
    typer.echo(f"Installed to {format_file_path(target)}")
    ...
    typer.echo(f"Linked    {format_file_path(alias_link)} -> agentworks")
...
def _install_zsh(script: str) -> None:
    ...
    typer.echo(f"Linked    {format_file_path(alias_link)} -> _agentworks")
...
def _install_powershell(script: str) -> None:
    ...
    # Absolute, deliberately, and NOT via `format_file_path`: this line is
    ...
    typer.echo(f"Added to $PROFILE: {format_file_path(profile_path)}")
...
def _remove_file(path: Path) -> bool:
    ...
    try:
        ...
        typer.echo(f"Warning: could not remove {format_file_path(path)}: {e}", err=True)
        return False
    typer.echo(f"Removed {format_file_path(path)}")
    return True
...
def _uninstall_powershell() -> None:
    ...
    if profile_path.exists():
        ...
        if len(kept) != len(lines):
            ...
            typer.echo(f"Removed source line from $PROFILE: {format_file_path(profile_path)}")
            removed = True
