...
def remove_config(target: Transport, ws_name: str, workspace_path: str) -> None:
    """Remove both managed tmuxinator projections for a disabled workspace."""
    config_path = shlex.quote(f"{workspace_path}/.tmuxinator.yml")
    session = shlex.quote(console_session_name(ws_name))
    target.run(
        f"rm -f {config_path} ~/.config/tmuxinator/{session}.yml",
        check=False,
        timeout=10,
    )
