...
git-credential manifest's ``spec.provider.name`` value. The framework's
``git-credential-provider`` kind holds one row per known provider so a typo in
the operator's provider name surfaces as a clean miss-policy error at
``build_registry`` time.
"""
...
def validate_git_tokens(
    providers: dict[str, GitCredentialProvider],
    tokens: dict[str, str],
) -> dict[str, str]:
    """Return a line-safe copy of the delivered token mapping."""
...
def build_credential_materials(
    ...
    tokens = validate_git_tokens(providers, tokens)
