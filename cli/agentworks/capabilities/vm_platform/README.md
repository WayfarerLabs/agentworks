...
naming the exact manual removal (Azure's `rollback_create_on_interrupt`; Lima and WSL2 do the same
over their single instance / distro). That is distinct from, and composes under, the orchestrator's
DB-row unwind (ADR 0019's `RealizationLog` / node `teardown`), which rolls back the persisted VM
row. Keep the two separate in your head: your sweep undoes backend-side resources you created inside
