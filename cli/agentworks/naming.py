"""The resource NAMING rule and the per-kind length caps.

One rule for what a resource may be called, and one cap per kind derived
at the module that owns that kind's downstream sink. Kept in its own
top-level module, next to ``declared_resource`` and for the same reason:
a declared row carries its ``name`` as a MODEL FIELD, and a model resolves
its field annotations at class-definition time, so a kind that validates
its names at load has to reach this rule without dragging in a package
that imports it back. ``agentworks.config`` is exactly such a package: its
``__init__`` loads the settings models, which import ``agentworks.secrets``,
whose row declares a capped name.

Split out of ``agentworks/config/validation.py``, which keeps the checks
that really are about the config file (paths, the admin username, the
workspaces root, mise inputs).
"""
