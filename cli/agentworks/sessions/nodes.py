...
class LiveSessionNode:
    ...
    def harness_integration(self) -> HarnessIntegration:
        """The held harness_integration instance, for the ``start`` call site.
        Readiness is composed through :meth:`preflight` /
...
def _harness_integration_for_template(
    ...
    *,
    ...
    on create (``claude-code``'s session id) survives to later starts. This
    ...
    philosophy (this seam only runs on the create/start/restart paths, so
