...
def config_init() -> None:
    ...
    if CONFIG_PATH.exists():
        typer.echo(f"Config already exists: {format_file_path(CONFIG_PATH)}")
    ...
    typer.echo(f"Sample config written to {format_file_path(CONFIG_PATH)}")
...
def config_edit() -> None:
    ...
    if not CONFIG_PATH.exists():
        typer.echo(f"Error: config file not found at {format_file_path(CONFIG_PATH)}", err=True)
...
def config_sync_vscode_workspaces() -> None:
    ...
    for ws in workspaces:
        ...
        typer.echo(f"  {ws.name} -> {format_file_path(path)}")
        count += 1

    typer.echo(f"Regenerated {count} VS Code workspace file(s) in {format_file_path(config.paths.vscode_workspaces)}")
