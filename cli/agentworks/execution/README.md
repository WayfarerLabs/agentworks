...
probe on Linux with CPython 3.12.13 interrupted construction after child creation and left that local
child alive without a returned handle. Launch-interruption ownership remains an unresolved production
gate, including for the existing SSH copy. A deadline consumes startup time but cannot interrupt an
OS process-creation call that has not returned; it is not a hard real-time bound over that call.
