...
_LATCHED_HEAL_HINT = (
    ...
    "forwarder (issue #117 latched state). Heal manually:\n"
    "\n"
    ...
    "\n"
    "IMPORTANT: the first command terminates your SSH session if you\n"
    "reach this VM over Tailscale (the agentworks default), and the\n"
    "rest of the sequence will not run. Either:\n"
    "\n"
    "  (a) Connect via a non-Tailscale transport before running the\n"
    "      block above. For Lima: `limactl shell <vm-name>`. For\n"
    "      WSL2: `wsl -d <distro>` from the Windows host. For Azure:\n"
    "      use the serial console from the Azure portal.\n"
    "\n"
    "  (b) Or, if you only have Tailscale access, fire the whole\n"
    "      sequence as a detached systemd unit so it survives the\n"
    "      disconnect, then reconnect after a few seconds:\n"
    "\n"
    "        sudo systemd-run --no-block bash -c '\n"
