"""Unit tests for ``agentworks.path_rendering.format_file_path``: the three
...
def test_format_file_path_uses_tilde_for_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: tmp_path))
    rendered = format_file_path(tmp_path / "agentworks" / "config.toml")
    assert rendered == "~/agentworks/config.toml"


def test_format_file_path_falls_back_to_absolute_outside_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
