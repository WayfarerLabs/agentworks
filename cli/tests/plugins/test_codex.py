...
def _config(tmp_path: Path, body: str = "", *, enabled: bool = False) -> Config:
    ...
    ["codex"]`` and ``body`` appends resource declarations."""
    pub = tmp_path / "id.pub"
    priv = tmp_path / "id"
...
_CODEX_INSTALL_TEMPLATE = """
[agent_templates.default]
user_install_commands = ["codex"]
"""
...
def test_template_referencing_codex_install_finalizes_when_disabled(tmp_path: Path) -> None:
    ...
    registry = build_registry(_config(tmp_path, _CODEX_INSTALL_TEMPLATE))
    assert registry.graph.enablement_of("user-install-command", "codex") is Enablement.disabled
...
def test_enabling_codex_lets_the_install_command_be_consumed(tmp_path: Path) -> None:
    registry = build_registry(_config(tmp_path, _CODEX_INSTALL_TEMPLATE, enabled=True))
