...
def test_sweep_predicts_a_harness_integration_config_secret_with_owner_usage_framing(
    ...
    monkeypatch.setitem(HARNESS_INTEGRATION_REGISTRY, "scanner", _SecretHarnessIntegration)
    template = [ManifestDoc("session-template", "scan", {"harness_integration": {"scanner": {}}})] if declared else []
...
def test_agent_template_node_derives_credential_edges(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    write_manifests(
        ...
        ManifestDoc("git-credential", "gh2", {"provider": {"name": "github", "source": {"mode": "secret"}}}),
