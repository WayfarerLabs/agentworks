...
def test_mode_options_work_on_group_and_subcommands_with_leaf_precedence() -> None:
    runner = CliRunner()
    listed = runner.invoke(app, ["guide", "list"])
    ...
    local_shown = [
        runner.invoke(app, ["guide", "show", topic, "--agent"]),
        runner.invoke(app, ["guide", "show", "--human", topic]),
    ]
    overridden = [
        runner.invoke(app, ["guide", "--human", "show", topic, "--agent"]),
        runner.invoke(app, ["guide", "--agent", "show", "--human", topic]),
    ]
    mode_lists = [
        ...
        for invocation in (
            ["guide", "--agent", "list"],
            ["guide", "list", "--agent"],
            ["guide", "--agent", "list", "--human"],
        )
    ]

    assert all(result.exit_code == 0 and result.stdout for result in [*selected, *shown, *local_shown, *overridden])
    assert selected[0].stdout != selected[1].stdout
    assert shown[0].stdout != shown[1].stdout
    assert [result.stdout for result in local_shown] == [result.stdout for result in shown]
    assert [result.stdout for result in overridden] == [result.stdout for result in shown]
