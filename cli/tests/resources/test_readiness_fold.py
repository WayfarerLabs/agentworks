...
def test_disabled_secret_backend_makes_its_active_source_not_ready() -> None:
    ...
    # and the vendor plugins' weak guest-CLI install-command rows; disable
    ...
    registry.finalize(
        enablement_sources=[
            _source_disabling(
                ...
                ("system-install-command", "gcloud-cli"),
...
def test_r9_9_mapping_is_validated_whether_or_not_its_backend_is_enabled(disable_onepassword: bool) -> None:
    ...
    def _build(mapping: str) -> Registry:
        ...
        # plugins' weak guest-CLI install-command rows (all emitted by
