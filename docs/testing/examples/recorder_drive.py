"""Example harness: REAL-CODE driver.
...
This example drives `agentworks.schema.extract_references`, which the
module docstring documents as total: it must never raise regardless of
input shape. The corpus below is illustrative (a handful of representative
"nice" and "adversarial" blobs), not exhaustive; the project's own property
test suite (tests/schema/test_extract_totality.py) is the maintained,
exhaustive version of this idea. Use this pattern when you want a quick,
readable, one-off check of a similar contract elsewhere in the codebase.

This is an illustrative example, not a maintained test suite.
"""
