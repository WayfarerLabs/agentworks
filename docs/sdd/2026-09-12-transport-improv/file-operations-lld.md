...
Same-controller owner canonicalization follows the
[generic recovery contract](execution-lifecycle-lld.md#durable-lifecycle-obligation-ledger). For
DOWNLOAD, an exact retry cannot bypass the admitted reconciliation or cleanup dispatch guard, while
separate controllers still require the full generation takeover plus adapter drain proof.
