...
## Phase 1: manifest-only plugin move

- [ ] Add and register the `apt` and `install-command` plugin descriptors with empty capabilities.
- [ ] Add one complete conceptual guide topic under each plugin package using eagerly constructed,
      inert contributions and consent-bearing action records for config edits and verification.
...
installed and disabled by default, and no model, reference, predicate, runner, or initializer moved.

## Phase 2: tests and operator teaching

- [ ] Preserve the exact payload oracle while changing its provider and enablement expectations.
- [ ] Cover descriptors, roster, default disablement, independent and combined enablement, use
      gating, operator override parity, doctor, filtered resource completion, discoverable guide
...
Definition of done: source tests prove unchanged enabled behavior and retained current disabled
behavior, while every permanent operator surface teaches the opt-in ownership accurately.
