...
FRD R10 is the authoritative transition contract. The operator waived the default warning runway on
2026-08-14; affected configurations use the standard disabled-resource message and manual plugin
enablement. Downgrade to 0.13 after adding either new plugin name is outside this effort's support
contract.

## Sequencing

1. Register both descriptors; move each manifest entry to its owning package; remove the old
   built-in files; and update the exact payload/provider oracle, descriptor-default, and provenance
   coverage in one green slice.
2. Add guide-scoped content loading and operator teaching with the import-boundary, package-data,
   and documentation checks in a second green slice.
...
| A manifest is missing from the wheel         | Extend installed-wheel coverage for both new YAML bundles; the existing Markdown glob covers guides. |
| Plugin import reads authored guide content   | Pin the I/O-free import boundary once; perform package reads only for a guide request.               |
| A custom apt package retains a moved source  | Rely on the generic gate tests and teach both existing-contract operator paths.                      |
