...
overlay. On a handled failure the existing partial-create cleanup removes the native owner and
buffer. A process crash retains the existing refusal to adopt unexplained native residue. Reinit of
an existing owner checkpoints directly into its existing instance-state row. This does not add
partial-create rows or a workspace repair/reinit command.
