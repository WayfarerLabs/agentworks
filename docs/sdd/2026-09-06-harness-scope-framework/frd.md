...
The operator's 2026-09-15 amendment makes every facet's selection a map keyed by integration name. A
setup resource can activate each integration once; an empty config value activates its defaults.
Template inheritance preserves parent integrations and merges a same-named integration's config
according to its facet schema. The session selection uses the same keyed shape while remaining
singular: a different selected integration replaces the previous selection. This change applies to
authored templates and instance specs together.
