...
1. No current column can establish historic applied state. In particular, `vms.ssh_public_key` is a
   legacy column that neither `insert_vm` writes nor `_to_vm` exposes
   (`cli/agentworks/db/database.py:292-326`, `cli/agentworks/db/converters.py:52-76`). Existing
   instances must therefore have unknown applied state until a real reinit or resume establishes a
   slice.
...
| `settings`               | `key`, `value`                                                                                                                                                                                                                                                                              | Exact key lookup. Doctor reads the system slug; VM and agent composition roots use it to build operation scope (`cli/agentworks/db/database.py:397-404`, `cli/agentworks/doctor_state.py:32-59`).                                                                                                                                                                                                                                  | Upsert by key. The first VM-create path records the operator's system-slug choice (`cli/agentworks/db/database.py:406-412`, `cli/agentworks/vms/manager/_helpers.py:163-208`).                                                                                                                                                                     |
...
### P5: legacy VM columns look more authoritative than they are
...
state in the same transaction as the owner row. It should not introduce a synthetic universal
instance table merely to obtain a foreign key. Owner-scoped reads may remain resilient to unrelated
corruption, but repository validation or doctor must report every orphaned record as database
damage. An absent applied record is a legitimate unknown; a present record with no owner is an
invariant violation.
...
- exact lookup by `(instance_kind, instance_name, record_kind, record_key)`;
...
- batch listing by instance kind and record kind for doctor and show surfaces;
...
Applied state is sliced rather than one all-or-nothing document. A VM create can record its resolved
spec and trusted SSH identity when those become facts. Reinit replaces only what it actually
reapplies. Sessions can later record a resume-established slice. Workspace repair records no
complete applied spec unless its contract grows into full convergence. Absence remains unknown.
...
| `record_kind`, `record_key`      | Separate desired overlay, applied spec slices, future integration applied-state, and later artifact records without predefining their payloads. |
| `schema_version`                 | Select the typed decoder for the payload. This is a record schema, not the database schema version.                                             |
...
The natural key is `(instance_kind, instance_name, record_kind, record_key)`. Add an index
supporting `(instance_kind, record_kind)` batch reads. Do not add columns for integration or
...
transaction. The repository owns SQL, the persistence envelope, record-kind registration, and
...
payload model and codec in its own domain and registers a typed record kind; it does not reopen
...
The two justified adjacent cleanups are transaction correctness for lifecycle checkpoints that write
new state and test trim on surfaces this effort rewrites. Schema-classifier or sentinel
