...
an ordering-preservation claim. A 100-site browser witness must keep these counts constant and show
no increasing event-listener count. Hosted CI does not gate browser wall-clock timings:
shared-runner scheduling is not a deterministic product invariant. The permanent witness instead
fixes the actual workload, bounded retained state, DOM, listeners, and collision instrumentation.
Performance may be profiled separately when diagnosing a regression.
