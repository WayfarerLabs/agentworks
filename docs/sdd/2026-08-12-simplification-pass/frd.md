...
- R1.1: A compact trust-boundary rule exists and states: validation belongs at real boundaries
  (operator-authored config and manifests, external processes and their outputs, network input,
  content from outside the repository); first-party typed code under mypy strict is trusted; every
  validator names the boundary it defends. `secrets/line_safety.py` is cited as the in-repo
  exemplar.
...
- R2.2: The website workflow-pinning test file is removed (W1); any single load-bearing fact it
  guarded is retained as a minimal check.
- R2.3: Remaining prose-policing assertions across guide, schema, database, platform, and website
  tests (G12, C10, D4, P6, W4, W6) are removed or converted to structural assertions, coordinated
  with the prose-test-purge effort (see Boundaries).
...
- R3.1: Runtime validation of first-party content and first-party code is removed: the guide
  contract's adversarial layer as applied to in-repo content (G1), the typed-to-dict re-parse round
  trip (G2), backend-protocol paranoia around in-repo backends (S2), the mypy-duplicating half of
  registration conformance (S2, C1, C5), and adversarial validation of our own parsers' outputs (S7,
  G6).
- R3.2: Mechanisms genuinely fit for a future external-plugin boundary are kept, cleaned, and
  ...
  registration-time constructibility checking, and the inert-action token discipline. Each surviving
...
- R7.4: Unexercised process machinery is trimmed or parked: the `review-requested` label and the
  stack section reduce to their operative content unless the operator directs otherwise (PR2).
...
- No SDD-directory cleanup (stale-SDD deletion is its own deliberate process).
- No absorption of the prose-test-purge effort (see Boundaries).
- No harness changes; rule-delivery fixes use in-repo surfaces only.

## Boundaries with the prose-test-purge effort

The seeded prose-test-purge SDD (2026-08-11) owns wording assertions on authored prose. This pass
owns the `phase7` corpus (form-policing of source shape, coupled to production-code removal, R2.1)
and contributes its findings (C10, D4, P6, G12, W4) to the purge effort's survey rather than
duplicating its sweep. Where a single file contains both concerns, this pass takes the whole file
and says so, because splitting one file across two efforts is worse than either whole option. The
effort leads coordinate through the saga/message conventions.
...
- 0.14 ships with the R5 breaks, the R6 migration notes, and no new compat layers.
