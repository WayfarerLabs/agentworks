...
- [x] Rename loader functions: `_load_task_config` to `_load_session_config`,
  `_load_task_templates` to `_load_session_templates`
- [x] Rename key sets: `_TASK_CONFIG_KEYS` to `_SESSION_CONFIG_KEYS`,
...
- [x] Remove `TASK_SEPARATOR` constant and `derive_session_name()` function -- session names are
...
- [x] `create_session` returns the socket path (or None for admin mode) so the caller can persist
  it to the DB
...
- [x] Rename all public functions: `create_task` to `create_session`, `stop_task` to
...
- [x] `_effective_socket_path()` helper to derive socket path for migrated sessions with
  NULL `socket_path`
...
from the agent's linux_user as a fallback when `socket_path` is NULL. On next `session restart`,
the socket path will be computed and persisted.
