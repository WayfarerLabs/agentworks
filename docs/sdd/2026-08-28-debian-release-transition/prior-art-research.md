...
- Lima exposes native snapshot create, list, apply, and delete commands using caller-assigned tags,
  ...
  validates the absolute sibling directories reported by Lima and performs a same-parent atomic
  directory move for each destructive boundary. The stopped clone already applies Lima's rules for
  omitting protection and runtime files and nullifying the VZ identifier; the original directory
  moves intact to the protected emergency name and is never started. A clone interrupted before
  `lima.yaml` exists cannot be removed by `limactl delete`, so its unreadable deterministic target
  remains fenced with exact repair guidance rather than being adopted or blindly removed. Clone does
  not make separately managed `additionalDisks` independent, so that case must fail rather than
  claim whole-VM recovery.
