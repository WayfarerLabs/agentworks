...
An incomplete artifact is listable or deletable only when the platform can still prove its ownership
metadata. If a provider exposes only a directory name while its metadata is unreadable, as Lima can
after a hard interruption during clone, the platform fences the lifecycle with exact inspection and
repair guidance. It never promotes the name alone into an owned descriptor or races a clone that may
still be running.
...
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Lima     | The live instance's driver selects the primitive. QEMU uses `limactl snapshot create/list/apply/delete`, including over SSH placement. VZ uses a stopped, protected `limactl clone`; restore clones a temporary instance, retains the exact original pre-first-restore directory under a protected emergency name, and uses an exclusive local OS rename to move the recovered sibling directory back to the original name without copy fallback. SSH-placed VZ create and restore refuse before mutation and direct the operator to a local-placement site on the Lima host. The checkpoint clone records the stable restore operation identity for replay. Delete validates and removes every related recovery instance. VZ refuses `additionalDisks`. |
