...
- [ ] Test WAL-visible copy, validation-before-destination-open, source/destination direction,
      ...
      deadline, restrictive POSIX modes, collision handling, cleanup, mixed-version retention
      ordering, manual retention immunity, confirmation without an implicit pre-restore backup,
      stdout purity, and all generated completion surfaces.
- DoD: on-demand backup and confirmed restore work without migration side effects; targeted tests,
  the complete gate, project-specific review, and mutations of copy direction and exact schema-shape
  validation all pass.
