...
- [ ] Obtain the saga lead's phased artifact ruling and confirm the singular `database` CLI home.
...
- [ ] Add the focused `agentworks.db` online-copy service with adjacent backup directory creation,
      ...
      incomplete-destination cleanup, and automatic-only retention of five.
...
- [ ] Add WAL-aware non-migrating schema inspection and safe writable-open sequencing to the same
      database safety service. Refuse malformed and future schema, complete any selected backup
      before `Database` construction, close construction failures, and raise a kind-based
...
- [ ] As the invoking session, save an `agw-state` snapshot before PR testing; fetch the PR's real
      head and base; prove the checkout matches the remote head; check freshness and merge conflicts
      with current `main`; and preserve the snapshot path until disposition and cleanup are
      complete.
- [ ] Run two mutation checks: force migration before backup, and force completion through a
      writable open. Each must fail a focused test; restore production behavior afterward.
...
- [ ] As the invoking session, load `integration-testing` and `agw-test-env`, select the local-only
      ...
      non-mutating completion. Post its evidence-backed disposition to the PR and independently
      verify that it left no residue.
- [ ] Obtain the saga lead's final implementation ruling after code, review, gate, and live-test
      findings converge.
- [ ] Classify residual migration claims and completion call sites, confirm the brief is deleted,
      update all plan checkboxes truthfully, and create `locked.md` containing final state and
      evidence.
...
- AC5: Phase 1 retention tests.
- AC6: Phase 1 restore tests and Phase 3 CLI validation.
