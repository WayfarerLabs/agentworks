...
- [ ] Add the focused `agentworks.db` online-copy service with adjacent backup directory creation,
      ...
      automatic-only retention of five.
...
- [ ] Test WAL-visible copy, validation-before-destination-open, source/destination direction,
      generic SQLite rejection, stable sentinels across every historical schema version,
      identical-path refusal, held-lock deadline, restrictive POSIX modes, collision handling,
      cleanup, mixed-version retention ordering, manual retention immunity, confirmation without an
      implicit pre-restore backup, stdout purity, and all generated completion surfaces.
...
- [ ] Add WAL-aware non-migrating schema inspection and safe writable-open sequencing to the same
      ...
      recheck schema under that lock, refuse malformed and future schema, complete any selected
...
- [ ] Add the hidden completion-probe marker. When sidecars exist, return no candidates; otherwise
      ...
      including when config warned or failed earlier. Completion must never prompt, migrate, create
      a database, or create or change SQLite sidecars.
...
- [ ] Test fresh/current/stale/future/malformed matrices; interactive accept and decline;
      ...
      command rendering; two-process serialized recheck with one backup; changed-but-stale waiter
      refusal; interactive JSON and names-only stdout purity; doctor; and byte-identical zero-output
      completion probes, including shell-wrapped warning and invalid-config cases.
...
- [ ] Run three mutation checks: force migration before backup, remove the serialized schema
