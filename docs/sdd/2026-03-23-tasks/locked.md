...
- The built-in default task template runs a login shell (empty command) rather than a specific
  tool like Claude. Claude is shown as an example in the sample config.
- `task start` was renamed to `task restart` with support for a `restart_command` template field.
- The restricted tmux config loads the user's `~/.tmux.conf` and selectively unbinds dangerous
  keys rather than stripping all keybindings.
- Tmuxinator was repurposed (not removed) as the workspace console mechanism. The VM console
  remains a separate, dynamically-built tmux session.
