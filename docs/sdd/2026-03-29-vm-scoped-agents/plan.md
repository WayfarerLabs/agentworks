...
- [x] Update `create_agent()`: `--vm` instead of `--workspace`, optional
...
- [x] Change workspace group naming from `ws-<name>` to `ws--<name>` (double hyphen, consistent
  with `agt--<name>`)
...
- [x] Verify no stale references to workspace-scoped agents, old username convention, or
