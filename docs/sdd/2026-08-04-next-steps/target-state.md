...
integration API carries one init method per facet (vm, user, workspace, where `user_init` runs for
...
at: vm, user, workspace, session; core owns the scope-to-facet mapping, and per-facet config is a
