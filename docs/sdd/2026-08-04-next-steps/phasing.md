...
- **0.14.0 (held; operator ruling, 2026-08-06):** the breaking cleanup does not ship alone. The cut
  ...
  default posture means manifests that never wrote the retired blocks cross without edits. The
  git-credential one-arm union restructure (operator ruling, 2026-08-08, ahead of credential
  minting) also lands before the cut, following the same pattern. While `main` holds unreleased
