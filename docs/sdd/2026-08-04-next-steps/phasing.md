...
- **0.14.0 (held; operator ruling, 2026-08-06):** the breaking cleanup does not ship alone. The cut
  ...
  ride the forgiving 0.13.0 until then. That gate is satisfied: the guide first slice merged
  2026-08-08 via PR #428. The installer-plugins child (operator ruling, 2026-08-07; launchable
  whenever, see Tracks) also gates the cut: its moves are breaking and belong in the same
  ...
  following the same pattern, along with any sibling variant restructures its survey task confirms
  against the variant-modeling contract in `target-state.md`. The secret-sources reference break
