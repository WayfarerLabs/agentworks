...
| ACLs                         | Canonical setfacl re-run every time; the step snapshots the tree's ACLs with `getfacl` before and after and compares, reporting `Fixed:` only on a real change and `OK:` otherwise.                                     |
| Parent traversal             | `chmod a+x` re-applied up each ancestor; carries `-c`, so the step reports `Fixed:` when it opened a missing traversal bit and `OK:` when every ancestor was already traversable.                                       |
