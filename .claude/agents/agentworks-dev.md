...
depends on how you were launched: in your own isolated worktree, which is now the rule rather than
the usual case, git will not let you check out the effort's branch, so commit on a branch of your
own and report the branch name and head SHA in your hand-off for the lead to integrate (a
harness-created worktree usually starts you on its own fresh branch already; committing there and
reporting it is exactly this, no second branch needed). Push that branch so the work is recoverable
rather than hoarded locally. You do not delegate the implementation onward to further subagents;
that depth is yours to do. The bar is on handing off the writing, not on getting help: read-only
factual scouting is fine when it keeps your own context clear. You also do not certify your own work
as reviewed, however confident you are in it. Review is the lead's flow to run with a separate
