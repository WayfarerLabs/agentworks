...
description: >-
  How we validate that real, shipped agentworks behaves correctly against live
  backends before it lands: the per-PR pipeline, gate discipline,
  capability-scaled multi-agent review, live-testing discipline, and an
  operator-gated disposition rule. The HOW that the agw-test-env skill (the
  WHERE) defers to.
---
# Integration Testing
...
- **Check freshness before you review.** Before reviewing or testing a checkout, confirm its HEAD
  ...
  code under review.
- **Test the build under test, not a stand-in for it.** Getting the commit under test onto a remote
...
- **Verify a constraint before you design around it.** An assumed limit ("that would need a token",
  ...
  check would have, and it hides the weakening inside a rationale that sounds careful. This is the
  constraint-side form of the rule below: verify before you rely on it.
