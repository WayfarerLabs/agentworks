...
__all__ = (
    ...
    "DOCUMENT_CONTRACTS",
    "FULL_MANIFEST",
    "GENERATED_PROOF_IMPORT",
    "GENERATED_PROOF_SOURCE",
    "COLLISION_SOURCE",
    "COLLISION_SOURCE_IMPORT",
    "MAIN_ATTRIBUTES",
    "MANIFESTO_CONTRACT",
    "PYPI_URL",
    "REPORTING_URL",
...
GENERATED_PROOF_SOURCE: Final = Path("static/lander-route-proofs.generated.js")
GENERATED_PROOF_IMPORT: Final = (
    'import { REFERENCE_PROOF_CATALOG } from "./lander-route-proofs.generated.js";\n'
)
COLLISION_SOURCE: Final = Path("static/lander-collision-source.js")
COLLISION_SOURCE_IMPORT: Final = (
    'import { compareExactRoots, exactRootNumber, exactSegmentContact, exactZeroRoot } from "./lander-collision-source.js";\n'
)


def _compose_lander_model(website: Path) -> str:
    """Compose reviewed generated proof bytes into the existing shipped model artifact."""
    model = _read_utf8(website / "static/lander-model.js")
    generated = _read_utf8(website / GENERATED_PROOF_SOURCE)
    if model.count(GENERATED_PROOF_IMPORT) != 1:
        raise ValueError("lander-model.js must have exactly one generated proof import")
    if not generated.startswith("// @generated by website/tools/project_lander_route_proofs.mjs; do not edit.\n"):
        raise ValueError("generated route proof source lacks its exact provenance marker")
    return f"{generated}\n{model.replace(GENERATED_PROOF_IMPORT, '')}"


def _compose_lander_world(website: Path) -> str:
    """Compose authored exact-collision authority into the existing shipped world artifact."""
    world = _read_utf8(website / "static/lander-world.js")
    collision = _read_utf8(website / COLLISION_SOURCE)
    if world.count(COLLISION_SOURCE_IMPORT) != 1:
        raise ValueError("lander-world.js must have exactly one collision-source import")
    return f"{collision}\n{world.replace(COLLISION_SOURCE_IMPORT, '')}"
...
def _render_artifact(repo_root: Path, site_base: str) -> tuple[dict[Path, bytes], frozenset[Path]]:
    ...
    copy_content[Path("static/lander-model.js")] = _compose_lander_model(website)
    copy_content[Path("static/lander-world.js")] = _compose_lander_world(website)
