...
element's exact `textContent` on user activation, and reports success or a manual fallback through a
polite status region without moving focus. It reads no clipboard data, makes no network request, and
provides no ongoing Agentworks guidance. Manifesto and Security have no JavaScript. Lander and the
custom 404 load only the same-origin game module.
