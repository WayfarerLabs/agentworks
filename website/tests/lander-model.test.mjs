...
test("all 100 keyed proof records replay every concrete terrain assignment", async () => {
    ...
    assert.equal(Object.keys(REFERENCE_TEMPLATE_CATALOG).length, 100);
    ...
    for (const record of fixture.records) {
        const proof = REFERENCE_TEMPLATE_CATALOG[record.pairKey];
        assert.ok(proof);
        ...
        const [, origin, target] = proof.pairKey.split(":").map(Number);
        assert.equal(
            proof.allowance,
            Math.ceil((12.65 + Math.max(0, target - origin) / 3000 - 1e-12) / FUEL_QUANTUM) * FUEL_QUANTUM,
        );
    }
    for (const assignment of fixture.assignments) {
        const proof = REFERENCE_TEMPLATE_CATALOG[assignment.pairKey];
        const context = assignmentContext(geometry, assignment);
        assert.equal(proveTemplate(proof, context), proof, assignment.assignmentId);
    }
    ...
    close(
        Math.ceil(Math.max(...REFERENCE_TEMPLATES.map(({ baseBurn }) => baseBurn)) / FUEL_QUANTUM) * FUEL_QUANTUM,
        12.65,
    );
...
test("accepted touchdown margins settle to one centered immutable checkpoint", async () => {
    ...
    for (const assignment of fixture.assignments.filter((_, index) => index % 257 === 0)) {
        ...
        for (const x of [
            ...
            assert.equal(
                proveTemplate(REFERENCE_TEMPLATE_CATALOG[assignment.pairKey], { ...context, pose }).success
                    .classification,
                "safe",
            );
        }
...
test("production route proof rejects a weak or inexact launch prefix before collision replay", async () => {
    ...
    const context = assignmentContext(
        geometry,
        fixture.assignments.find(({ pairKey }) => pairKey === template.pairKey),
    );
    assert.throws(
        () => proveTemplate({ ...template, runs: [[2, 90], ...template.runs.slice(1)] }, context),
        /must begin with exact \[1,90\] launch request/,
    );
    assert.throws(
        () => proveTemplate({ ...template, runs: [[1, 89], ...template.runs.slice(1)] }, context),
        /must begin with exact \[1,90\] launch request/,
    );
});
