...
class StaticDocumentTests(unittest.TestCase):
    ...
    def test_scene_geometry_and_start_target_are_fixed_and_responsive(self) -> None:
        ...
        self.assertIn((312, 515.2), surface)
        self.assertIn((322, 519.04), surface)
        self.assertIn((400, 544), surface)
        self.assertIn((410, 547.1999999999999), surface)
        self.assertIn((488, 566.4), surface)
        self.assertIn((498, 562.4), surface)
        deck = next(attributes for _, attributes in self.document.tags if attributes.get("class") == "landing-platform")
        self.assertEqual(float(deck["y"]), 490.2)
        self.assertEqual(float(deck["y"]) + float(deck["height"]), 493.7)
        support = next(attributes for _, attributes in self.document.tags if attributes.get("class") == "site-scaffold")
        self.assertTrue(support["d"].startswith("M312 493.7H498M312 501.2H498"))
        self.assertEqual(support["d"].count("M"), 61)
        ...
        self.assertTrue(noc["d"].startswith("M428 493.7V"))
