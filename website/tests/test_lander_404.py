...
class StaticDocumentTests(unittest.TestCase):
    ...
    def test_scene_geometry_and_start_target_are_fixed_and_responsive(self) -> None:
        ...
        self.assertIn((312, 353.6), surface)
        self.assertIn((322, 351.52), surface)
        self.assertIn((400, 332.8), surface)
        self.assertIn((410, 330.4), surface)
        self.assertIn((488, 312.32), surface)
        self.assertIn((498, 310.72), surface)
        deck = next(attributes for _, attributes in self.document.tags if attributes.get("class") == "landing-platform")
        self.assertEqual(float(deck["y"]), 285.72)
        self.assertEqual(float(deck["y"]) + float(deck["height"]), 289.22)
        support = next(attributes for _, attributes in self.document.tags if attributes.get("class") == "site-scaffold")
        self.assertTrue(support["d"].startswith("M312 289.22H498M312 296.72H498"))
        ...
        self.assertTrue(noc["d"].startswith("M428 289.22V"))
