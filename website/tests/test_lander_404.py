...
class StaticDocumentTests(unittest.TestCase):
    ...
    def test_scene_geometry_and_start_target_are_fixed_and_responsive(self) -> None:
        ...
        self.assertIn((312, 377.4294401056133), surface)
        self.assertIn((322, 377.3153997545617), surface)
        self.assertIn((400, 376.4258850163594), surface)
        self.assertIn((410, 376.25661170741915), surface)
        self.assertIn((488, 374.9362798976854), surface)
        self.assertIn((498, 374.7670065887452), surface)
        deck = next(attributes for _, attributes in self.document.tags if attributes.get("class") == "landing-platform")
        self.assertEqual(float(deck["y"]), 232)
        self.assertEqual(float(deck["y"]) + float(deck["height"]), 235.5)
        support = next(attributes for _, attributes in self.document.tags if attributes.get("class") == "site-scaffold")
        self.assertTrue(support["d"].startswith("M312 235.5H498M312 243H498"))
        self.assertEqual(support["d"].count("M"), 131)
        ...
        self.assertTrue(noc["d"].startswith("M428 235.5V"))
