...
class StaticDocumentTests(unittest.TestCase):
    ...
    def test_scene_geometry_and_start_target_are_fixed_and_responsive(self) -> None:
        ...
        self.assertIn((312, 509.44), surface)
        self.assertIn((322, 512.48), surface)
        self.assertIn((400, 531.1999999999999), surface)
        self.assertIn((410, 533.6), surface)
        self.assertIn((488, 551.68), surface)
        self.assertIn((498, 553.28), surface)
        deck = next(attributes for _, attributes in self.document.tags if attributes.get("class") == "landing-platform")
        self.assertEqual(float(deck["y"]), 484.44)
        self.assertEqual(float(deck["y"]) + float(deck["height"]), 487.94)
        support = next(attributes for _, attributes in self.document.tags if attributes.get("class") == "site-scaffold")
        self.assertTrue(support["d"].startswith("M312 487.94H498M312 495.44H498"))
        self.assertEqual(support["d"].count("M"), 57)
        ...
        self.assertTrue(noc["d"].startswith("M428 487.94V"))
