...
- Status: Draft pending operator answers to the five open questions (§5)
- Date: 2026-08-10
...
| `resume`                                                                    | Restart a session workload preserving its logical identity, running harness resume hooks. Deliberately not `start` (§4).                                                                                                                                                                                                                                                                                                  |
...
| `backup`                                                                    | Export an archive of an instance. (`agw database backup/restore` arrives via safer-migrations; `vm restore` deferred, §4.)                                                                                                                                                                                                                                                                                                |
...
| `guide` / `doctor` / `completion` / `version` / `config init\|edit\|sample` | System verbs; shapes unchanged (guide ruled on in §4).                                                                                                                                                                                                                                                                                                                                                                    |
...
9. Exit-code + confirmation audit across all commands against rules 5-7 — CHEAP, mostly
   verification.
...
- **No batch `--all` on vm/workspace/agent/console lifecycle ops**: DECLINE by cardinality —
  sessions are fleets; VMs are few and batch delete is a footgun; VM idle-stop belongs to the
  observability wave's auto-suspend.
