...
A merged FRD is the operator's however much of it the lead drafted, so correcting one is an
amendment request (`sdd` skill, Artifact Mutability).
...
directly, and findings on a merged FRD go to the operator as an amendment request.
