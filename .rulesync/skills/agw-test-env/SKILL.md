...
name: agw-test-env
description: >-
  The generic, parameterized shape of an agentworks live integration-test environment: inventory
  framework, naming, budgets, and safety protocol, with operator-specific values left as
  placeholders any operator fills in. Load before invoking agentworks-tester agents or doing any
  live testing, and inject the relevant sections into each tester's prompt.
targets: ["*"]
---
