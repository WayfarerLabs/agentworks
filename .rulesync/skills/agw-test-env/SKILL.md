...
name: agw-test-env
description: >-
  The agentworks live integration-test environment on this host: inventory, naming, budgets, and
  safety protocol. Load before invoking agentworks-tester agents or doing any live testing, and
  inject the relevant sections into each tester's prompt.
targets: ["*"]
---
...
carries one. Fill them in from your own environment, or load a machine-specific, gitignored
companion skill (this repo's own test host uses one named `agw-test-env-local`, generated only to
this machine's own harnesses and never committed) that supplies them alongside this one.
...
Substitute your own value wherever a placeholder appears in the body:
...
- **Scope review lanes deliberately on large or foundational PRs.** A wide multi-agent review
  ...
  callers.
- **PR intervention is all-or-nothing.** When live-validating a PR, either take the entire PR over
  the finish line yourself (fix every finding, get gates green, and say so explicitly), or push zero
  commits and send every finding back to the dev, no matter how small. Decide before the first
  commit; mixed mode (fixing some things while asking the dev for the rest) silently diverges the
  remote branch under a dev who may not notice the new commits. If you do go all in after fetching a
