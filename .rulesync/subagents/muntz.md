...
targets: ["*"]
description: >-
  Pushes back on bad complexity per the project's development principles. Invoke on a design, plan,
  diff, PR, or existing surface when the question is whether its complexity earns its keep. Never
  changes the work under review; experiments stay in its own worktree. Produces a written verdict
  per finding.
claudecode:
  model: inherit
---
