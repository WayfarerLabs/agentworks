...
description:
  ...
  in the same change, and keeps guide teaching safe"
---
...
path, the relevant readiness or enablement states, and the next safe action, without restating facts
the guide already projects from the finalized registry or stored instance rows. The second is a
security boundary, not a style preference: guide content is instructional, never consent, so it must
not resolve or expose secrets, inspect the workstation, connect to a VM, perform remote work, mutate
